...
export class AddLanguageComponent implements OnInit {
    ...
    public fileConfig = new FileConfig(
        {
            allowedFileExtensionList: ['jpg', 'jpeg', 'png', 'bmp', 'gif', 'tif', 'tiff', 'eps', 'svg'],
    ...
    constructor(@Inject(MAT_DIALOG_DATA) public languageModel: LanguageModel,
    ...
    setLanguageDetails() {
        if (this.languageModel) {
            ...
            this.frmAddNew.controls.LanguageCode.setValue(this.languageModel.languageCode);
            this.frmAddNew.controls.LanguageDefaultDisply.setValue(this.languageModel.languageDefaultDisply);
            this.frmAddNew.controls.LanguageDirection.setValue(this.languageModel.languageDirection);
            this.frmAddNew.controls.languageFlag.setValue(this.languageModel.languageFlag);

            this.languagesService.getLanguageById(this.languageModel.id).pipe(
                mergeMap((resultLanguageFlag: LanguageModel) => {
                    return (resultLanguageFlag.languageFlag) ?
                        this.documentService.getUploadedDocument(resultLanguageFlag.languageFlag) : of('');
                }),
                tap((binaryImage: string) => {
                    return (binaryImage) ?
                        this.setLanguageFlagImage(binaryImage) : of('');;
                }),
            ).subscribe(result => { });
        }
    }
    ...
    OnSubmit() {
        ...
        var initialObservable = this.fileListFormControl.value ? this.uploadFlag() : of({});
        return initialObservable.pipe(
            mergeMap((uploadedDocuments: IUploadedDocument[]) => {

                if (this.fileListFormControl.value)
                    this.frmAddNew.controls.languageFlag.setValue(uploadedDocuments[0].url);

                return (this.ID === 0) ? this.languagesService.addLanguage(this.frmAddNew.value)
                    : this.languagesService.updateLanguage(this.frmAddNew.value)
            }),
            catchError(() => of(null)))
            .subscribe((id: any) => {
                if (id) {
    ...
    public uploadFlag(): Observable<IUploadedDocument[]> {
        return this.documentService.uploadDocuments(this.fileListFormControl.value);
    }
    private validateFiles(fileList: FileList): string[] {
        return this.fileService.buildValidationSuccessMessageList(fileList, this.fileConfig);
    }
